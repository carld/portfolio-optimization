...
returns <- function(data){
  dat <- data[[1]]; N <- nrow(dat) - 1; J <- ncol(dat); ret <- (dat[1:N,1] - dat[2:(N+1),1])/dat[2:(N+1),1]; if(J > 1){
      for(j in 2:J){
...
optimization <- function(returns){
  p <- colMeans(returns[[1]]); names(p) <- returns[[2]]; J <- ncol(returns[[1]]); M <- returns[[3]];
  Rmax <- returns[[4]];
  S <- cov(returns[[1]]); Q <- solve(S); u <- rep(1,J); a <- matrix(rep(0,4),nrow=2);
  a[1,1] <- u%*%Q%*%u;
  a[1,2] <- a[2,1] <- u%*%Q%*%p;
  a[2,2] <- p%*%Q%*%p;
...
plot_results<- function(data, returns, results){
  ...
  minp <- results[[5]]; tanp <- results[[6]]; wminp <- results[[7]]; wtanp <- results[[8]]; f <- t(results[[9]]); v <- results[[10]];
  sv <- results[[11]]; rv <- results[[12]]; postscript(file="./results1/fig1.eps", onefile=FALSE,
  ...
  text(sv[1], rv[1], pos=4, cex=0.5, "DEP"); points(minp[1], minp[2], pch=19, col="black"); text(minp[1], minp[2], pos=2, cex=0.5, "MVP1"); points(tanp[1], tanp[2], pch=19, col="black"); text(tanp[1], tanp[2], pos=2, cex=0.5, "TGP");
  lines(c(0,max(s)), c(0,max(s)*tanp[2]/tanp[1]), lty=3); abline(h=0, lty=2); abline(v=0, lty=2);
  ...
  abline(h=0, lty=2); abline(v=minp[1], lty=3); abline(v=tanp[1], lty=3);
  ...
  postscript(file="./results1/fig3.eps", onefile=FALSE,
              horizontal=FALSE, height=10, width=5);
  par(mfrow=c(2,1));
  barplot(wminp, main="Minimum Variance Portfolio 1 (MVP1)", xlab="Assets", ylab="Weights",
